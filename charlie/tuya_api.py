...
class TuyaCloudAPI:
    """Tuya 2B 开发者云 API 客户端 (HMAC-SHA256 签名鉴权)。

    用于红外遥控器 (category=qt) 等 2C shadow API 无法控制的设备——2C 终端用户
    API (sk- Bearer key) 的 shadow/properties/issue 只更新影子不触发红外发码，
    必须走 2B 红外云 API: /v2.0/infrareds/{infrared_id}/air-conditioners/
    {remote_id}/scenes/command。

    凭证: TUYA_CLIENT_ID / TUYA_ACCESS_KEY (云应用 Access ID/Key, 与 2C sk- key 不同套)。
    签名: stringToSign = method + "\\n" + sha256(body).lower() + "\\n\\n" + url;
          sign = HMAC-SHA256(access_key, client_id + access_token + t + stringToSign).upper()。
    body 的 sha256 必须与实际发送字节一致 (requests json= 发送 json.dumps(body), 故
    签名里也用 json.dumps(body), 二者默认 separators 一致)。
    """

    def __init__(self, access_id: str = None, access_key: str = None,
                 base_url: str = None, timeout: int = 30):
        if access_id is None:
            access_id = os.environ.get("TUYA_CLIENT_ID")
        if access_key is None:
            access_key = os.environ.get("TUYA_ACCESS_KEY")
        if not access_id or not access_key:
            raise ValueError(
                "Missing 2B credentials. Set TUYA_CLIENT_ID and TUYA_ACCESS_KEY "
                "(cloud project Access ID/Key, distinct from the 2C sk- API key)."
            )
        if base_url is None:
            base_url = os.environ.get("TUYA_BASE_URL", "https://openapi.tuyacn.com")
    ...
    def _string_to_sign(self, method: str, path: str, params: dict = None,
    ...
    def _signed_headers(self, method: str, path: str, params: dict = None,
                        body: dict = None) -> dict:
        string_to_sign, t = self._string_to_sign(method, path, params, body)
        message = self.access_id + self.access_token + t + string_to_sign
        sign = hmac.new(self.access_key.encode("utf-8"), message.encode("utf-8"),
    ...
    def _request(self, method: str, path: str, params: dict = None,
                 body: dict = None, _is_token_call: bool = False):
        if not _is_token_call and time.time() >= self._token_expire - 60:
            self._refresh_token()
        headers = self._signed_headers(method, path, params, body)
        url = f"{self.base_url}{path}"
        if method == "GET":
    ...
    def _refresh_token(self):
        # 加锁防并发雪崩: 多请求同时过期时只刷新一次
        with self._token_lock:
            # double-check: 等锁期间可能已被其他线程刷新
            if self.access_token and time.time() < self._token_expire - 60:
                return
            ...
            self._token_expire = time.time() + result.get("expire_time", 7200)

    # ─── Infrared Air Conditioner ───

    def get_ir_remotes(self, infrared_id: str) -> list:
        """List all remotes paired under an infrared gateway (infrared_id)."""
        return self._request("GET", f"/v2.0/infrareds/{infrared_id}/remotes")

    def get_ac_remote_id(self, infrared_id: str) -> str:
        """Find the air conditioner remote_id (category_id == 5). None if not found."""
        for r in self.get_ir_remotes(infrared_id):
            if r.get("category_id") == 5:
                return r.get("remote_id")
        return None

    def ac_scenes_command(self, infrared_id: str, remote_id: str,
                          power: int, mode: int = None, temp: int = None,
                          wind: int = None) -> bool:
        """Issue air conditioner commands (triggers IR send) via single-key endpoint.

        Uses /command (single key per call) instead of /scenes/command — the latter
        returns 30100 "no code in library" for some brand/temp combos (e.g. Mitsubishi).
    ...
    def ac_status(self, infrared_id: str, remote_id: str) -> dict:
        """Query the air conditioner's last-sent state (power/mode/temp/wind)."""
