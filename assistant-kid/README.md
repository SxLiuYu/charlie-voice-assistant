...
## v3.0 新增功能
- **流式语音闭环**: 大脑逐句产出→TTS批量→SSE推送 (首文本1.7s/首音频3.5s)
- **自动监听**: 声音检测→自动录音→静默停止→防回声cooldown
- **大脑断路器**: 连续失败3次自动重建, /api/brain/restart手动重启
- **内存保护**: RAM>90%拒绝构建大脑, 防OOM崩溃
- **可配置MCP**: MCP_SERVERS环境变量, 默认4个核心服务器(RAM 77%→66%)
- **请求指标**: /api/metrics (请求数/错误率/p50/p95/各端点明细)
- **SSE心跳保活**: 5s间隔, 防止大脑思考时连接断开
- **共享utils模块**: 时间解析/临时文件清理/历史截断/错误脱敏
- **线程安全**: _history_lock全链路保护
- **JSON解析保护**: 4处req.json()加try/except
- **错误响应脱敏**: 移除内部路径/密钥/IP信息
- **定期临时文件清理**: 每10分钟+启动时
- **对话历史文件保护**: >100条自动截断
...
- **唤醒词检测**: 浏览器端'Charlie'唤醒(webkitSpeechRecognition)
  - 唤醒后自动启动录音, 无需点击
